...
let store_object _obj = let list = obj_to_list _obj in
  let digest = hash _obj in
    Out_channel.with_open_gen [Open_creat; Open_trunc; Open_text; Open_wronly] 777 (Sys.getcwd () ^ "/.ogit/objects/" ^ (Digest.to_hex digest)) (fun file -> Out_channel.output_string file (String.concat "\n" list)); digest
...
let rec restore_work_directory _obj = let rec loop = function
    ...
    | (name, is_dir, _, obj) :: rest -> if is_dir then
      begin
        ...
        Out_channel.with_open_gen [Open_creat; Open_trunc; Open_text; Open_wronly] 777 name (fun file -> Out_channel.output_string file (match obj with Text s -> s | _ -> failwith "Invalid object")); loop rest
  in loop ((match _obj with Directory dir -> dir | _ -> failwith "Invalid object"))

let merge_work_directory_I _obj = false