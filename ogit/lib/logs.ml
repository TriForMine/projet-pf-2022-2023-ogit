...
let set_head _l = Out_channel.with_open_gen [Open_creat; Open_trunc; Open_text; Open_wronly] 0o777 (Sys.getcwd () ^ "/.ogit/HEAD") (fun file -> (List.map Digest.to_hex _l) |> (String.concat ";") |> Out_channel.output_string file )

let get_head () = In_channel.with_open_text (Sys.getcwd () ^ "/.ogit/HEAD") (fun file -> In_channel.input_all file |> String.split_on_char ';' |> List.map Digest.from_hex)
...
let store_commit _c = let file_content = Printf.sprintf "%s\n%s\n%s\n%s" (_c.parents |> List.map Digest.to_hex |> String.concat ";") (date_fm _c.date) _c.message (Digest.to_hex _c.content) in
